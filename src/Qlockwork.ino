...
#define FIRMWARE_VERSION 20241105
...
// Brightness and LDR
uint8_t maxBrightness_during_daytime = map(settings.mySettings.brightness_during_daytime, 0, 100, MIN_BRIGHTNESS, MAX_BRIGHTNESS);
uint8_t brightness_during_daytime = maxBrightness_during_daytime;
uint8_t brightness = maxBrightness_during_daytime;
#ifdef WEATHER
uint8_t maxBrightness_during_nighttime = map(settings.mySettings.brightness_during_nighttime, 0, 100, MIN_BRIGHTNESS, MAX_BRIGHTNESS);
uint8_t brightness_during_nighttime = maxBrightness_during_nighttime;
#endif
#ifdef LDR
uint16_t ldrValue = 0;
...
void setup()
{
	...
	pinMode(PIN_MODE_BUTTON_LED, OUTPUT);
    ...
    else
    {
        ...
        Serial.println("isDay: " + String(outdoorWeather.isDay));
#endif
        if (outdoorWeather.isDay)
        {
            brightness = brightness_during_daytime;
        #ifdef DEBUG
            Serial.println("Day brightness set.");
        #endif
        }
        else
        {
            brightness = brightness_during_nighttime;
        #ifdef DEBUG
            Serial.println("Night brightness set.");
        #endif
        }
#endif
    }
...
void loop()
{
    ...
    if (minute() != lastMinute)
    {
...
#ifdef WEATHER
        // Set night- and day brightness
        if ((hour() == hour(timeZone.toLocal(outdoorWeather.sunset + NIGHT_BRIGHTNESS_DELAY))) && (minute() == minute(timeZone.toLocal(outdoorWeather.sunset + NIGHT_BRIGHTNESS_DELAY))))
        {
#ifdef DEBUG
            Serial.println("Night brightness set.");
#endif
        ...
        if ((hour() == hour(timeZone.toLocal(outdoorWeather.sunrise - NIGHT_BRIGHTNESS_DELAY))) && (minute() == minute(timeZone.toLocal(outdoorWeather.sunrise - NIGHT_BRIGHTNESS_DELAY))))
        {
#ifdef DEBUG
            Serial.println("Day brightness set.");
#endif
        ...
        if (minute() % 5 == 0)
        {
      		digitalWrite(PIN_MODE_BUTTON_LED,LOW);
#ifdef SYSLOGSERVER_SERVER
...
{
  digitalWrite(PIN_MODE_BUTTON_LED, HIGH);
#ifdef DEBUG
...
void handleButtonSettings()
{
    ...
    {
        message += "<option value=\"" + String(i) + "\"";
        if (i == settings.mySettings.brightness_during_daytime)
            message += " selected";
        message += ">";
        message += String(i) + "</option>";
    }
    ...
    message += "<tr><td>"
        ...
        "</td><td>"
        "<select name=\"br_night\">";
    for (int i = 10; i <= 100; i += 10)
    {
        message += "<option value=\"" + String(i) + "\"";
        if (i == settings.mySettings.brightness_during_nighttime)
            message += " selected";
        message += ">";
        message += String(i) + "</option>";
    }
...
void handleCommitSettings()
{
    ...
    if (webServer.arg("ab") == "0")
    {
        ...
        brightness = maxBrightness_during_daytime;
    }
    else
        settings.mySettings.useAbc = true;
#endif
    // ------------------------------------------------------------------------
    settings.mySettings.brightness_during_daytime = webServer.arg("br_day").toInt();
    maxBrightness_during_daytime = map(settings.mySettings.brightness_during_daytime, 0, 100, MIN_BRIGHTNESS, MAX_BRIGHTNESS);
    brightness = maxBrightness_during_daytime;
#ifdef WEATHER
    settings.mySettings.brightness_during_nighttime = webServer.arg("br_night").toInt();
    maxBrightness_during_nighttime = map(settings.mySettings.brightness_during_nighttime, 0, 100, MIN_BRIGHTNESS, MAX_BRIGHTNESS);
    brightness_during_nighttime = maxBrightness_during_nighttime;
    if (outdoorWeather.isDay)
    {
        brightness = maxBrightness_during_daytime;
    }
    ...
    {
        brightness = maxBrightness_during_nighttime;
    }
#endif
    // ------------------------------------------------------------------------
