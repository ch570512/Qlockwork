...
uint8_t MeteoWeather::getOutdoorConditions(String lat, String lon, String timezone) {
    ...
    if (client.connect("api.open-meteo.com", 80)) {
        String url = "/v1/forecast?latitude=" + String(lat) + "&longitude=" + String(lon) + "&current=is_day,temperature_2m,relative_humidity_2m,surface_pressure&daily=sunrise,sunset&timeformat=unixtime&timezone=" + timezone + "&forecast_days=1";
        ...
        bool current_isDay = current["isDay"]; // 0
